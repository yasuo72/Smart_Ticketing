...
export async function enrichTicketWithAi(ticketId: string, subject: string, description: string) {
  try {
    ...
    const data = classification.autoResolvable
      ? await buildAutoResolveData(ticketId, classification.autoReply)
      : {};
...
async function buildAutoResolveData(ticketId: string, autoReply: string | null) {
  if (!autoReply) {
    return {};
  }
