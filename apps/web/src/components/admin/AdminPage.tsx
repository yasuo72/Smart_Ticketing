        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              Share this password securely with the new {role.toLowerCase()}.
            </p>
          </div>
    ...
    >
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-2xl animate-scale-in">
        <div className="p-6 text-center">
          ...
          >
            <AlertTriangle
              className={cn('size-7', isDangerous ? 'text-red-600' : 'text-amber-600')}
            />
          </div>
...
export function AdminPage() {
  ...
  async function updateUser(id: string, patch: Partial<Pick<AuthUser, 'role' | 'isActive'>>) {
    ...
    if (!res.ok) {
      setUserError('Could not update user.');
      return;
    }
  ...
  async function confirmDeactivateUser() {
    ...
    if (!res.ok) {
      setUserError('Could not deactivate user.');
      return;
    }
    const data = (await res.json()) as { user: AuthUser };
    setUsers((prev) => prev.map((u) => (u.id === data.user.id ? data.user : u)));
  ...
  async function confirmDeleteTicket() {
    ...
    if (!res.ok) {
      setTicketError('Could not delete ticket.');
      return;
    }
  ...
  const filteredTickets = tickets.filter(
    (t) => !ticketSearch.trim() || t.subject.toLowerCase().includes(ticketSearch.toLowerCase()),
  );
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[
            ...
            return (
              <div
                key={stat.label}
                className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm card-hover"
              >
